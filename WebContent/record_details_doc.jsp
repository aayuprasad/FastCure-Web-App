...
<body style="background-color: yellow; background-image: url('./bg.jpg'); background-size: 100% 100%; background-repeat: no-repeat; background-attachment: fixed;">     
    <h1 style="position: absolute; left: 800px; color: rgb(10, 10, 10);"> Patient Record Details</h1>
    <%
	...
	ResultSet rs3 = st3.executeQuery("SELECT pp.pno from patient_pno pp WHERE pp.patient_id = "+rs.getString(2));
    ...
    <%  session.setAttribute("recordid",rs.getString(1));
        session.setAttribute("pname",rs.getString(2));
    %>

    <div home style="float: right; width: 25%;">
        </form>   
        <form action="./health_tracker_doc.jsp" method="get">
            <input type="submit" value="View Patient's Health Tracker" />
        </form> 
        <form action="./add_pres.html" method="get">
            <input type="submit" value="Add Prescription" />
        </form> 
        <form action="./predict.jsp" method="get">
          <input type="hidden"name="record_id"value=<%= rs.getString(1)%>>    
          <button type="submit">See Predicted Diseases</button>  
        </form> 
    </div> 
</body>     
</html>  